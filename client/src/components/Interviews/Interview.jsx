...
function Interview({ interveiwData, onFinish }) {
    ...
    useEffect(() => {
        ...
        recognition.onerror = (event) => {
            ...
            if (event.error === "network") {
                console.log("Network issue - restarting mic...");
                setTimeout(() => {
                    if (isMicOnRef.current) {
                        try {
                            recognition.start();
                        } catch {
                            console.log("Mic already started or failed to restart");
                        }
                    }
                }, 1000);
            }
            ...
            if (event.error === "aborted") {
                console.log("Mic aborted (normal)");
            }
        };

        recognition.onend = () => {
            // Only restart if user explicitly wants mic ON
            if (isMicOnRef.current && !isAIPlaying) {
                try {
                    recognition.start();
                } catch {
                    console.log("Failed to restart mic after end");
    ...
    const startMic = () => {
        ...
        setTimeout(() => {
            ...
            } catch { console.log("Mic already started") }
        }, 200);
    };
    ...
    const stopMic = () => {
        ...
        try {
            ...
            console.log("Mic already stopped or failed to stop");
        }
    };
    ...
    const toggleMic = () => {
        ...
        if (isAIPlaying) return; // prevent mic during AI speaking
        ...
        from-emerald-50 via-white to-teal-100 flex items-center justify-center p-4 sm:p-6'>
            ...
            shadow-2xl border border-gray-200 flex flex-col lg:flex-row overflow-hidden'>
                ...
                <div className='flex-1 flex flex-col p-4 sm:p-6 md:p-8 relative'>
                    {isMicOn && (
                        <p className="text-green-600 text-sm mb-2 animate-pulse">
                            🎤 Listening... Speak now
                        </p>
                    )}
                    ...
                    <textarea
                        placeholder={isMicOn ? "Listening... speak your answer" : "Type your answer here..."}
...
//         const runIntro = async () => {
//             if (isIntroPhase) {
//                 await speakText(`Hi ${userName}, it's great to meet you.`)
