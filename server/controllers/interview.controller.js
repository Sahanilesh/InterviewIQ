    ...
    try {
        ...
        if (!role || !experience || !mode) {
            return res.status(400)
                .json({ message: "Role, Experience and mode sre required." })
        }
...
`
        ...
        if (!aiResponse || !aiResponse.trim()) {
            return res.status(500).json({ messages: "AI captured empty response." })
        }
        ...
        if (questionsArray.length === 0) {
            return res.status(500).json({ messages: "AI failed to generate questions." })
        }
        user.credits -= 50;
        await user.save()
...
export const finishInterview = async (req, res) => {
    try {
        ...
        const avgConfidenec = totalQuestions ? totalConfidence / totalQuestions : 0
        ...
        return res.status(200).json({
            ...
            confidence: Number(avgConfidenec.toFixed(1)),
...
export const getMyInterviews = async (req, res) => {
    try {
        ...
        return res.status(500).json({message: `Failed to fetch interview history: ${error.message}`})
    }
}
...
export const getInterviewReport = async (req, res) => {
    try {
        ...
        if(!interview){
            return res.status(404).json({message: "Interview not found with given id."})
        }
